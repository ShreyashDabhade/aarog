...
const DoctorDashboard = () => {
  ...
  const fetchData = useCallback(
    async (isBackground = false) => {
      ...
      try {
        ...
        if (!resp.ok) throw new Error('Failed to fetch shared reports');
  ...
  const handleViewFile = useCallback(
    async (report) => {
      if (!userPrivateKey) {
        alert('Session key missing. Please relogin.');
        return;
      }
      ...
      try {
        try {
          ...
          if (!onChainHash || onChainHash === '') {
            alert('Warning: this file is not registered on the blockchain.');
          } else {
  ...
  useEffect(() => {
    if (!autoViewId || sharedReports.length === 0) return;
  ...
  }, [autoViewId, handleViewFile, sharedReports, verifying]);
    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <StatCard
          ...
          value={sessionCode && timeLeft > 0 ? 'Yes' : 'No'}
          color="bg-indigo-500"
        />
      </div>
            ...
            <div className="relative z-10">
              ...
              {sessionCode && timeLeft > 0 ? (
                <div className="text-center animate-in zoom-in bg-white/5 p-4 rounded-2xl border border-white/10">
                  <div className="text-4xl font-mono font-bold tracking-widest text-emerald-400 mb-2">
                    {sessionCode}
                  </div>
              ...
              {Object.keys(patients).length === 0 ? (
                ...
                Object.keys(patients).map((email) => (
                  <button
                    ...
                    className={`w-full text-left p-4 border-b border-slate-50 flex items-center justify-between hover:bg-blue-50 transition-colors ${
                      selectedPatient === email ? 'bg-blue-50 border-l-4 border-l-blue-500' : ''
                    }`}
                  >
              ...
              <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/30">
                <div>
                  ...
                  <p className="text-slate-500 text-sm">
                    Shared by <span className="font-semibold text-blue-600">{selectedPatient}</span>
                  </p>
                </div>
              ...
              <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-4">
                {patients[selectedPatient].map((report) => (
                  <div
                    ...
                    className="p-5 border border-slate-200 rounded-2xl hover:border-blue-300 hover:shadow-md transition-all group bg-white"
                  >
                    ...
                    <h4 className="font-bold text-slate-800 mb-1 truncate" title={report.filename}>
                      {report.filename}
                    </h4>
