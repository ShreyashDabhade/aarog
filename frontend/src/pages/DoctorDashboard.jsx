...
import { Users, Activity, FileText, Search, Clock, Link as LinkIcon, RefreshCw, ChevronRight, Lock, Eye, AlertTriangle } from 'lucide-react';
...
const DoctorDashboard = () => {
  ...
  // Polling Reference to prevent memory leaks
  ...
  const fetchData = async (isBackground = false) => {
    ...
    try {
      ...
      // Only update state if data changed (Simple length check or deep comparison could be better)
      // For MVP, we just update. React handles DOM diffing.
  ...
  useEffect(() => {
    ...
    pollingRef.current = setInterval(() => {
      fetchData(true); // true = background mode (no spinner)
    }, 5000);
