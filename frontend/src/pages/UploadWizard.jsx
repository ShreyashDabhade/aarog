    ...
    <div className="h-full flex flex-col justify-center animate-in fade-in zoom-in duration-300">
      <div className="border-3 border-dashed border-slate-200 rounded-3xl p-16 text-center hover:bg-blue-50 hover:border-blue-400 transition-all cursor-pointer relative group bg-slate-50/50">
        ...
        <p className="text-slate-500 max-w-sm mx-auto">
          Drag and drop your scanned reports (Images/PDF) here to begin the secure ingestion pipeline.
        </p>
      </div>
    </div>
  );
...
const OCRReview = () => {
  ...
  useEffect(() => {
    ...
    const runOCR = async () => {
      ...
      try {
        ...
        const response = await fetch(`${API_URL}/ocr/extract?mode=table`, {
          method: 'POST',
        ...
        if (!response.ok) {
          ...
          throw new Error(detail.detail || 'OCR request failed');
        }
  ...
  }, [file, token, setIsProcessing, setRawText]);
      ...
      <div className="bg-slate-100 rounded-2xl overflow-hidden flex items-center justify-center border border-slate-200 relative group">
        {file && file.type.includes('image') ? (
          <img
            src={processedImage}
            alt="Preview"
            className="max-h-[500px] object-contain opacity-90 group-hover:opacity-100 transition-opacity"
          />
        ) : (
      ...
      <div className="flex flex-col justify-center space-y-6">
        <div>
          ...
          <p className="text-slate-500 mt-2">
            Running backend PaddleOCR with image preprocessing for medical reports.
          </p>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex-1 max-h-[400px] overflow-hidden relative">
          {isProcessing ? (
            ...
            <pre className="font-mono text-xs text-slate-600 leading-relaxed overflow-y-auto h-full p-2 whitespace-pre-wrap break-words m-0">
...
const Anonymizer = () => {
  ...
  useEffect(() => {
    ...
    runNER();
  }, [rawText, setAnonymizedText, setIsProcessing]);
    ...
    <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-8 duration-300">
      ...
      <div className="grid grid-cols-2 gap-6 flex-1 min-h-0">
        <div className="flex flex-col h-full">
          ...
          <textarea
            readOnly
            value={rawText}
            className="flex-1 p-5 rounded-2xl border-2 border-red-100 bg-red-50/30 resize-none focus:outline-none text-sm leading-relaxed font-mono text-slate-600"
          />
        </div>
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="relative flex-1">
            ...
            <textarea
              ...
              onChange={(event) => setAnonymizedText(event.target.value)}
      ...
      <div className="mt-6 flex justify-end">
        <button
          onClick={() => setStep(4)}
          disabled={isProcessing}
          className="bg-slate-900 text-white px-10 py-4 rounded-xl font-bold hover:bg-black transition shadow-xl flex items-center gap-3"
        >
...
const EncryptAndUpload = () => {
  ...
  useEffect(() => {
    ...
    const process = async () => {
      ...
      try {
        const serverKeyResp = await fetch(`${API_URL}/server_pubkey.pem`);
    ...
    process();
  }, [anonymizedText, file, setReportId, setUploadStatus, token, uploadStatus, userPublicKeyPem]);
    ...
    return (
      <div className="text-center py-20 animate-in zoom-in duration-500">
        ...
        <div className="flex gap-4 justify-center">
          ...
          <button
            onClick={() => navigate('/')}
            className="px-8 py-3 border border-slate-300 rounded-xl hover:bg-slate-50 font-bold text-slate-600 transition"
          >
            Return to Vault
          </button>
          <button
            onClick={() => navigate('/chat')}
            className="px-8 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 shadow-lg shadow-blue-500/30 transition"
          >
            Chat with AI Agent
          </button>
        </div>
...
const UploadWizard = () => {
  const step = useUploadStore((state) => state.currentStep);
          ...
          <div key={index} className="flex items-center">
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center font-bold transition-all duration-500 ${
                step >= index
                  ? 'bg-blue-600 text-white scale-110 shadow-blue-500/40 shadow-lg'
                  : 'bg-slate-200 text-slate-400'
              }`}
            >
              {step > index ? <Check className="w-5 h-5" /> : index}
            </div>
            {index < 4 && (
              <div
