    ...
    <div className="flex h-screen bg-[#F8FAFC] overflow-hidden font-sans">
      ...
      >
        ...
        <div className="h-24 flex items-center px-8">
          ...
          <button
            className="ml-auto lg:hidden p-1 hover:bg-slate-800 rounded-full"
            onClick={() => setIsMobileMenuOpen(false)}
          >
        ...
        <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
          ...
          <SidebarItem
            ...
            label={role === 'doctor' ? 'Command Center' : 'My Vault'}
          />
          ...
          <SidebarItem
            to="/chat"
            icon={MessageSquare}
            label={role === 'doctor' ? 'Patient Query AI' : 'AI Assistant'}
          />
        </nav>
        ...
        <div className="p-4 bg-slate-900 border-t border-slate-800/50">
          ...
          {/* NEW: Consent & Emergency button for patients */}
          {role === 'patient' && (
            <button
              onClick={() => navigate('/consent')}
              className="mb-3 flex items-center justify-center gap-2 px-4 py-3 text-slate-200 bg-slate-800/60 hover:bg-slate-700/80 rounded-xl transition-all w-full font-medium text-sm border border-slate-700/60 hover:border-emerald-400/70 shadow-sm"
            >
              <Shield className="w-4 h-4" />
              <span>Privacy, Consent & Emergency</span>
            </button>
          )}
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 hidden sm:block">
                Manage your secure medical records
              </p>
            </div>
          </div>
