...
export const cryptoService = {
  ...
  // Encrypts text or ArrayBuffer (handles both for your Dual Upload)
  encryptData: async (aesKey, data) => {
    let encoded = data;
  ...
  decryptData: async (aesKey, cipherHex, ivHex, isText = true) => {
    ...
    return decryptedBuffer; // Return raw buffer for files (PDF/Images)
  },
  ...
  importPublicKey: async (pem) => {
    const binaryDer = pemToArrayBuffer(pem);
    return window.crypto.subtle.importKey("spki", binaryDer, { name: "RSA-OAEP", hash: "SHA-256" }, false, ["encrypt"]);
  },

  wrapKeyWithRSA: async (pemOrKey, aesKey) => {
    let rsaKey = pemOrKey;
    if (typeof pemOrKey === 'string') {
      rsaKey = await cryptoService.importPublicKey(pemOrKey);
    }
